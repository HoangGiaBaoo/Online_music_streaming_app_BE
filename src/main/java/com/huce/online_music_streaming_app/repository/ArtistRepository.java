...
public interface ArtistRepository extends JpaRepository<Artist, Long> {
    ...
    @Query("SELECT a, COUNT(f) AS followers FROM Artist a " +
           "LEFT JOIN FollowedArtist f ON f.artist = a " +
           "WHERE EXISTS (SELECT 1 FROM Track t JOIN t.genres g WHERE t.artist = a AND g.genreId = :genreId) " +
           "GROUP BY a ORDER BY followers DESC")
    List<Object[]> findByGenreIdOrderByFollowers(@Param("genreId") Long genreId, Pageable pageable);
}
